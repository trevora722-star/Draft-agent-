...
(`anna-and-james.confettialbum.com`) or the organizer's own custom domain.

## How it fits together

- **Main site** (`confettialbum.com`): landing page, organizer signup/login,
...
- **Event sites** (`<slug>.confettialbum.com` or a custom domain): the guest
...
- **Hosts are admins**: an organizer signed in on the main site
  ...
  session cookie is scoped to `.confettialbum.com`). Deletes are soft — files
...
|---|---|---|
| `CR_BASE_DOMAIN` | yes in prod | The platform's base domain, e.g. `confettialbum.com` |
...
1. Buy `confettialbum.com` and point it at the app host:
   - `confettialbum.com` -> your Render service (A/ALIAS record)
   - `*.confettialbum.com` -> the same service (wildcard CNAME). The wildcard
     is what makes every event subdomain work with zero per-event setup.
2. On Render, add both `confettialbum.com` and `*.confettialbum.com` as custom
...
name to `confettialbum.com`. For HTTPS on their domain, the domain must also
